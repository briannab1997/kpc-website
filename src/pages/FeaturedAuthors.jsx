...
export default function FeaturedAuthors() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const all = authors || [];
      setAvailableAuthors(all.filter(a => a.status === "available_now"));
      setUpcomingAuthors(all.filter(a => a.status === "upcoming"));
      setForthcoming2026Authors(all.filter(a => a.status === "forthcoming_2026"));
    } catch (error) {
  ...
  const handleCreateAuthor = async (status = "available_now") => {
    ...
    try {
      ...
      const maxOrder = allOrders.length ? Math.max(...allOrders) + 1 : 0;
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            {[
              ...
              { key: "hero_subtitle", label: "Hero Subtitle", multiline: true, rows: 3 },
              { key: "available_title", label: "Available Now Section Title", multiline: false },
              { key: "upcoming_title", label: "Upcoming Section Title", multiline: false },
              { key: "forthcoming_title", label: "Forthcoming 2026 Section Title", multiline: false },
              { key: "diversity_title", label: "Diversity Statement Title", multiline: false },
              { key: "diversity_text", label: "Diversity Statement Text", multiline: true, rows: 4 },
              { key: "cta_title", label: "CTA Title", multiline: false },
              { key: "cta_text", label: "CTA Text", multiline: true, rows: 2 },
            ].map(({ key, label, multiline, rows }) => (
              <div key={key}>
                <label className="text-sm font-medium">{label}</label>
                {multiline ? (
                  <Textarea rows={rows} value={pageContent[key]} onChange={(e) => setPageContent(p => ({ ...p, [key]: e.target.value }))} />
                ) : (
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setEditingAuthor(a => ({ ...a, name: e.target.value }))}
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setEditingAuthor(a => ({ ...a, country: e.target.value }))}
              />
            </div>
            <div>
              ...
              <Textarea
                rows={6}
                value={editingAuthor?.background || ""}
                onChange={(e) => setEditingAuthor(a => ({ ...a, background: e.target.value }))}
              />
            </div>
            <div className="flex justify-end gap-2 pt-4">
